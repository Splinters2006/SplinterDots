...
impl Paths {
    fn new() -> Self {
        ...
        Self {
            ...
            hypr_schema_file: root.join("config/dotfiles/hyprland-options.json"),
        }
    }

    fn repo_settings(&self) -> PathBuf {
        self.root.join("config/dotfiles/settings.conf")
    }
...
impl CenterApp {
    ...
    fn value(&self, key: &str) -> String {
        self.values.get(key).cloned().unwrap_or_else(|| default_value(key))
    }
    ...
    fn save_startup_choice(&self) -> Result<(), String> {
        ...
        if self.no_show_on_startup {
            fs::write(&self.paths.disabled_file, "").map_err(to_string)?;
        } else if self.paths.disabled_file.exists() {
            fs::remove_file(&self.paths.disabled_file).map_err(to_string)?;
        }
        Ok(())
    }
    ...
    fn tab_button(ui: &mut egui::Ui, selected: bool, text: &str) -> bool {
        let height = if selected { 40.0 } else { 30.0 };
        ui.add_sized(Vec2::new(128.0, height), egui::SelectableLabel::new(selected, RichText::new(text).strong())).clicked()
    }

    fn tab_bar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            if Self::tab_button(ui, self.tab == Tab::Overview, "Overview") { self.tab = Tab::Overview; }
            if Self::tab_button(ui, self.tab == Tab::Hyprland, "Hyprland") { self.tab = Tab::Hyprland; }
            if Self::tab_button(ui, self.tab == Tab::Shortcuts, "Shortcuts") { self.tab = Tab::Shortcuts; }
            if Self::tab_button(ui, self.tab == Tab::Appearance, "Appearance") { self.tab = Tab::Appearance; }
            if Self::tab_button(ui, self.tab == Tab::Bar, "QML Bar") { self.tab = Tab::Bar; }
            if Self::tab_button(ui, self.tab == Tab::Setup, "Setup") { self.tab = Tab::Setup; }
        });
    }
    ...
    fn overview_tab(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("overview_shortcuts").num_columns(3).spacing([12.0, 10.0]).show(ui, |ui| {
    ...
    fn hyprland_tab(&mut self, ui: &mut egui::Ui) {
        ...
        for option in &self.hypr_schema {
            grouped.entry(option.section.clone().unwrap_or_else(|| "Other".to_string())).or_default().push(option.clone());
        }

        ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
            for (section, options) in grouped {
                egui::CollapsingHeader::new(RichText::new(section).strong()).default_open(true).show(ui, |ui| {
                    for option in options { self.hypr_option_row(ui, &option); }
                });
            }
        });
    }
    ...
    fn hypr_option_row(&mut self, ui: &mut egui::Ui, option: &HyprOption) {
        ...
        let label = option.label.clone().unwrap_or_else(|| path.replace(':', " / ").replace('_', " "));
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| match kind.as_str() {
                ...
                "choice" => {
                    ...
                    egui::ComboBox::from_id_source(format!("hypr_{path}")).selected_text(current.clone()).width(160.0).show_ui(ui, |ui| {
                        for choice in choices { ui.selectable_value(current, choice.clone(), choice); }
                    });
                }
                "int" => {
                    let min = option.min.unwrap_or(0.0) as i64;
                    let max = option.max.unwrap_or(9999.0) as i64;
                    let mut value = current.parse::<i64>().unwrap_or(min).clamp(min, max);
                ...
                "float" => {
                    ...
                    let mut value = current.parse::<f64>().unwrap_or(min).clamp(min, max);
                    if ui.add(egui::DragValue::new(&mut value).speed(0.05).clamp_range(min..=max)).changed() {
                        *current = trim_float(value);
                    }
                }
                _ => {}
            });
    ...
    fn shortcuts_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            if ui.button("Add shortcut").clicked() {
                self.shortcuts.push(default_shortcut());
            ...
            if ui.button("Remove selected").clicked() {
                if let Some(index) = self.selected_shortcut {
                    if index < self.shortcuts.len() { self.shortcuts.remove(index); }
        ...
        ui.columns(2, |columns| {
            ScrollArea::vertical().max_height(430.0).show(&mut columns[0], |ui| {
                for (index, sc) in self.shortcuts.iter().enumerate() {
                    let selected = self.selected_shortcut == Some(index);
                    if ui.selectable_label(selected, format!("{}    {}", format_shortcut_key(&sc.key), sc.name)).clicked() {
            ...
            columns[1].group(|ui| {
                ...
                if let Some(index) = self.selected_shortcut {
                    if index < self.shortcuts.len() {
                        ...
                        ui.label("Type");
                        egui::ComboBox::from_id_source("shortcut_kind").selected_text(friendly_kind(&self.shortcut_edit.kind)).show_ui(ui, |ui| {
                            if ui.selectable_label(self.shortcut_edit.kind == "app", "App / script").clicked() { self.shortcut_edit.kind = "app".to_string(); }
                            if ui.selectable_label(self.shortcut_edit.kind == "desktop", "Desktop action").clicked() { self.shortcut_edit.kind = "desktop".to_string(); }
                        });
    ...
    fn appearance_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_source("theme").selected_text(&theme).show_ui(ui, |ui| {
        ...
        text_row(ui, &mut self.values, "Accent color", "DOTFILES_ACCENT");
        text_row(ui, &mut self.values, "Wallpaper", "DOTFILES_WALLPAPER");
        ui.horizontal(|ui| {
            if ui.button("Apply wallpaper").clicked() { self.apply_wallpaper(); }
            if ui.button("Apply theme").clicked() { self.save_all(); }
        });
    }
    ...
    fn bar_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ScrollArea::vertical().show(ui, |ui| {
            ui.group(|ui| {
                ui.heading("Layout");
                combo_row(ui, &mut self.values, "Position", "DOTFILES_BAR_POSITION", &["top", "bottom"]);
            });
            ui.group(|ui| {
                ui.heading("Modules");
                for (label, key) in [
                    ...
                    if ui.checkbox(&mut checked, label).changed() { self.set_bool_value(key, checked); }
                }
            });
            ui.group(|ui| {
                ui.heading("Clock");
                text_row(ui, &mut self.values, "Format", "DOTFILES_BAR_CLOCK_FORMAT");
                let mut seconds = self.bool_value("DOTFILES_BAR_CLOCK_SECONDS");
                if ui.checkbox(&mut seconds, "Show seconds").changed() { self.set_bool_value("DOTFILES_BAR_CLOCK_SECONDS", seconds); }
            });
            ui.group(|ui| {
                ui.heading("Workspaces");
                combo_row(ui, &mut self.values, "Workspace count", "DOTFILES_BAR_WORKSPACE_COUNT", &["5", "6", "7", "8", "9", "10", "12", "15", "20"]);
                let mut accent = self.bool_value("DOTFILES_BAR_ACCENT_WORKSPACE");
                if ui.checkbox(&mut accent, "Accent active workspace").changed() { self.set_bool_value("DOTFILES_BAR_ACCENT_WORKSPACE", accent); }
            });
            ui.group(|ui| {
                ui.heading("Sizing and style");
                slider_row(ui, &mut self.values, "Height", "DOTFILES_BAR_HEIGHT", 24.0, 72.0);
                slider_row(ui, &mut self.values, "Rounding", "DOTFILES_BAR_RADIUS", 0.0, 28.0);
                slider_row(ui, &mut self.values, "Opacity", "DOTFILES_BAR_OPACITY", 0.2, 1.0);
                slider_row(ui, &mut self.values, "Font size", "DOTFILES_BAR_FONT_SIZE", 8.0, 24.0);
                slider_row(ui, &mut self.values, "Spacing", "DOTFILES_BAR_SPACING", 4.0, 40.0);
                slider_row(ui, &mut self.values, "Border", "DOTFILES_BAR_BORDER_WIDTH", 0.0, 4.0);
            });
            ui.horizontal(|ui| {
                if ui.button("Save and restart bar").clicked() { self.save_and_restart_bar(); }
                if ui.button("Restart bar only").clicked() { self.restart_bar(); }
            });
        });
    }
    ...
    fn setup_tab(&mut self, ui: &mut egui::Ui) {
        ui.heading("Setup");
        text_row(ui, &mut self.values, "Terminal", "DOTFILES_TERMINAL");
        text_row(ui, &mut self.values, "File manager", "DOTFILES_FILE_MANAGER");
        text_row(ui, &mut self.values, "App launcher", "DOTFILES_APP_LAUNCHER");
        text_row(ui, &mut self.values, "Editor", "DOTFILES_EDITOR");
        text_row(ui, &mut self.values, "Browser", "DOTFILES_BROWSER");
        ui.add_space(8.0);
        ui.horizontal(|ui| {
            if ui.button("Open dotfiles folder").clicked() { run_quiet("xdg-open", &[self.paths.root.to_string_lossy().as_ref()]); }
            if ui.button("Reload Hyprland").clicked() { run_quiet("hyprctl", &["reload"]); }
        });
    }
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("bottom").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.button(RichText::new("Save changes").strong()).clicked() { self.save_all(); }
...
fn default_value(key: &str) -> String {
    DEFAULTS.iter().find(|(k, _)| *k == key).map(|(_, v)| (*v).to_string()).unwrap_or_default()
}
...
fn default_shortcut() -> Shortcut {
    Shortcut { name: "New shortcut".to_string(), key: "SHIFT, N".to_string(), kind: "app".to_string(), value: "kitty".to_string() }
}
...
fn sc(name: &str, key: &str, kind: &str, value: &str) -> Shortcut {
    Shortcut { name: name.to_string(), key: key.to_string(), kind: kind.to_string(), value: value.to_string() }
}

fn load_shortcuts(paths: &Paths) -> Vec<Shortcut> {
    let Ok(content) = fs::read_to_string(&paths.keybinds_json) else { return default_shortcuts(); };
...
fn save_shortcuts(paths: &Paths, shortcuts: &[Shortcut]) -> Result<(), String> {
    ensure_dir(&paths.dotfiles_dir)?;
    let text = serde_json::to_string_pretty(shortcuts).map_err(to_string)?;
    fs::write(&paths.keybinds_json, text + "\n").map_err(to_string)
}

fn load_hypr_schema(paths: &Paths) -> Vec<HyprOption> {
    let Ok(content) = fs::read_to_string(&paths.hypr_schema_file) else { return Vec::new(); };
    let Ok(schema) = serde_json::from_str::<HyprSchemaFile>(&content) else { return Vec::new(); };
    schema.options.into_iter().filter(|option| {
        let kind = option.kind.as_deref().unwrap_or("text");
        matches!(kind, "bool" | "choice" | "int" | "float") && !EXCLUDED_HYPR_OPTIONS.contains(&option.path.as_str())
    }).collect()
}
...
fn load_hypr_values(paths: &Paths, schema: &[HyprOption]) -> HashMap<String, String> {
    ...
    if let Ok(content) = fs::read_to_string(&paths.hypr_values_json) {
        if let Ok(saved) = serde_json::from_str::<HashMap<String, String>>(&content) {
            for (key, value) in saved { values.insert(key, value); }
        }
    }
    values
}
...
fn write_keybinds(paths: &Paths, values: &HashMap<String, String>, shortcuts: &[Shortcut]) -> Result<(), String> {
    ...
    for i in 1..=9 { lines.push(format!("bind = $mainMod, {i}, workspace, {i}")); }
    lines.push(String::new());
    for i in 1..=9 { lines.push(format!("bind = $mainMod SHIFT, {i}, movetoworkspace, {i}")); }
...
fn hypr_key_parts(key: &str) -> (String, String) {
    ...
    if key.to_uppercase().starts_with("SHIFT,") {
        ("$mainMod SHIFT".to_string(), key.split_once(',').map(|(_, k)| k.trim()).unwrap_or("").to_string())
    } else {
...
fn write_hyprland_settings(paths: &Paths, schema: &[HyprOption], values: &HashMap<String, String>) -> Result<(), String> {
    ...
    for option in schema {
        if !hypr_option_supported(&option.path) { continue; }
        let raw = values.get(&option.path).cloned().or_else(|| option.default.clone()).unwrap_or_default();
        let value = clean_hypr_value(option, &raw);
        if value.is_empty() { continue; }
        insert_hypr_value(&mut root, &option.path, value);
    }
    let mut lines = vec!["# Generated by Dotfiles Center.".to_string(), "# This file is safe to regenerate.".to_string(), String::new()];
    render_hypr_node(&root, 0, &mut lines);
    fs::write(&paths.hypr_generated, lines.join("\n") + "\n").map_err(to_string)
}

fn hypr_option_supported(path: &str) -> bool {
    if env::var_os("HYPRLAND_INSTANCE_SIGNATURE").is_none() { return true; }
    Command::new("hyprctl").args(["getoption", path]).stdout(Stdio::null()).stderr(Stdio::null()).status().map(|s| s.success()).unwrap_or(true)
}

fn insert_hypr_value(root: &mut HyprNode, path: &str, value: String) {
    let mut node = root;
    for part in path.split(':') { node = node.children.entry(part.to_string()).or_default(); }
...
fn render_hypr_node(node: &HyprNode, indent: usize, lines: &mut Vec<String>) {
    for (key, child) in &node.children {
        ...
        if child.children.is_empty() {
            if let Some(value) = &child.value { lines.push(format!("{prefix}{key} = {value}")); }
        } else {
...
fn clean_hypr_value(option: &HyprOption, value: &str) -> String {
    match option.kind.as_deref().unwrap_or("text") {
        "bool" => if is_true(value) { "true".to_string() } else { "false".to_string() },
        "choice" => {
            let choices = option.choices.clone().unwrap_or_default();
            if choices.contains(&value.to_string()) { value.to_string() } else { option.default.clone().unwrap_or_default() }
        }
        "int" => {
            let min = option.min.unwrap_or(0.0) as i64;
            let max = option.max.unwrap_or(9999.0) as i64;
        ...
        "float" => {
            let min = option.min.unwrap_or(0.0);
            let max = option.max.unwrap_or(9999.0);
            trim_float(value.parse::<f64>().unwrap_or(min).clamp(min, max))
        }
...
fn write_colors(paths: &Paths, values: &HashMap<String, String>) -> Result<(), String> {
    ...
    fs::write(&paths.hypr_colors, colors).map_err(to_string)?;
    let mako = format!(
        "background-color={}\ntext-color={}\nborder-color={}\nborder-size=2\nborder-radius=8\npadding=12\ndefault-timeout=5000\n",
        palette.background, palette.text, palette.accent,
    );
    fs::write(&paths.mako_config, mako).map_err(to_string)
}
...
fn theme_palette(values: &HashMap<String, String>) -> Palette {
    ...
    if value_or(values, "DOTFILES_THEME") == "light" {
        Palette { accent, background: "#f8fafc".to_string(), surface: "#e2e8f0".to_string(), text: "#0f172a".to_string(), muted: "#475569".to_string(), inactive_border: "#94a3b8".to_string(), bar_rgb: "f8fafc".to_string(), active_text: "#ffffff".to_string() }
    } else {
        Palette { accent, background: "#1e1e2e".to_string(), surface: "#313244".to_string(), text: "#cdd6f4".to_string(), muted: "#bac2de".to_string(), inactive_border: "#45475a".to_string(), bar_rgb: "1e1e2e".to_string(), active_text: "#11111b".to_string() }
    }
}
...
fn write_quickshell_bar(paths: &Paths, values: &HashMap<String, String>) -> Result<(), String> {
    ...
    if is_true(&value_or(values, "DOTFILES_BAR_CLOCK_SECONDS")) && !clock_format.contains("%S") { clock_format.push_str(":%S"); }
    let bg_color = format!("#{:02x}{}", (opacity * 255.0) as i32, palette.bar_rgb);

    let left_section = if is_true(&value_or(values, "DOTFILES_BAR_SHOW_WORKSPACES")) {
        let active_color = if is_true(&value_or(values, "DOTFILES_BAR_ACCENT_WORKSPACE")) { palette.accent.clone() } else { palette.surface.clone() };
        format!(r#"
          Row {{
            ...
            Repeater {{
              ...
              Rectangle {{
                ...
                Text {{ anchors.centerIn: parent; text: index + 1; color: (index + 1) === root.activeWorkspace ? "{active_text}" : "{text}"; font.bold: true; font.pixelSize: {workspace_font} }}
    ...
    let center_section = if is_true(&value_or(values, "DOTFILES_BAR_SHOW_CLOCK")) {
        format!(r#"
          Text {{
            ...
            text: "Loading..."
            Process {{ id: dateProc; command: ["date", {clock_arg}]; running: true; stdout: StdioCollector {{ onStreamFinished: clock.text = this.text.trim() }} }}
            Timer {{ interval: 1000; running: true; repeat: true; onTriggered: dateProc.running = true }}
          }}"#, text = palette.text, clock_arg = json_string(&format!("+{clock_format}")))
    } else { String::new() };

    let mut status_parts = Vec::new();
    if is_true(&value_or(values, "DOTFILES_BAR_SHOW_VOLUME")) { status_parts.push("printf 'VOL '; wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null | awk '{v=int($2*100); if($3==\"[MUTED]\") print \"MUTE\"; else print v\"%\"}'"); }
    if is_true(&value_or(values, "DOTFILES_BAR_SHOW_NETWORK")) { status_parts.push("printf '  NET '; nmcli -t -f GENERAL.STATE device show 2>/dev/null | grep -q ':100' && echo on || echo off"); }
    if is_true(&value_or(values, "DOTFILES_BAR_SHOW_BATTERY")) { status_parts.push("bat=$(cat /sys/class/power_supply/BAT0/capacity 2>/dev/null || cat /sys/class/power_supply/BAT1/capacity 2>/dev/null); [ -n \"$bat\" ] && printf '  BAT %s%%' \"$bat\""); }
    if is_true(&value_or(values, "DOTFILES_BAR_SHOW_CPU")) { status_parts.push("cpu=$(top -bn1 | grep 'Cpu(s)' | awk '{print int($2+$4)\"%\"}' 2>/dev/null); [ -n \"$cpu\" ] && printf '  CPU %s' \"$cpu\""); }
    if is_true(&value_or(values, "DOTFILES_BAR_SHOW_MEMORY")) { status_parts.push("mem=$(free -m | awk '/^Mem/{printf \"%dMB\", $3}' 2>/dev/null); [ -n \"$mem\" ] && printf '  MEM %s' \"$mem\""); }
    let status_cmd = if status_parts.is_empty() { "echo ''".to_string() } else { status_parts.join("; ") };

    let right_section = if status_parts.is_empty() { String::new() } else {
        format!(r#"
          Text {{
            id: status
            ...
            font.pixelSize: {font_size}
            text: ""
            Process {{ id: statusProc; command: ["sh", "-c", {status_cmd}]; running: true; stdout: StdioCollector {{ onStreamFinished: status.text = this.text.replace(/\n/g, "").trim() }} }}
            Timer {{ interval: 3000; running: true; repeat: true; onTriggered: statusProc.running = true }}
          }}"#, muted = palette.muted, status_cmd = json_string(&status_cmd))
    };

    let qml = format!(r#"// SplinterDots Quickshell bar.
...
Variants {{
  ...
  delegate: Component {{
    PanelWindow {{
      ...
      screen: modelData
      anchors {{ top: {top}; bottom: {bottom}; left: true; right: true }}
      margins {{ top: {top_margin}; bottom: {bottom_margin}; left: 10; right: 10 }}
...
"#,
        top = if position == "top" { "true" } else { "false" },
        bottom = if position == "bottom" { "true" } else { "false" },
        top_margin = if position == "top" { 8 } else { 0 },
        bottom_margin = if position == "bottom" { 8 } else { 0 },
        height = height,
        radius = radius,
        bg_color = bg_color,
    ...
    fs::write(&paths.quickshell_file, qml).map_err(to_string)
}

fn value_or(values: &HashMap<String, String>, key: &str) -> String { values.get(key).cloned().unwrap_or_else(|| default_value(key)) }
fn json_string(value: &str) -> String { serde_json::to_string(value).unwrap_or_else(|_| "\"\"".to_string()) }
fn is_true(value: &str) -> bool { matches!(value.to_ascii_lowercase().as_str(), "1" | "true" | "yes" | "on") }
fn friendly_kind(kind: &str) -> &'static str { if kind == "app" { "App / script" } else { "Desktop action" } }

fn format_shortcut_key(key: &str) -> String {
    let parts: Vec<String> = key.split(',').map(|part| match part.trim().to_ascii_uppercase().as_str() {
        "SHIFT" => "Shift".to_string(),
        "CTRL" => "Ctrl".to_string(),
        "ALT" => "Alt".to_string(),
        _ => part.trim().to_string(),
    }).filter(|part| !part.is_empty()).collect();
    if parts.is_empty() { "Super".to_string() } else { format!("Super + {}", parts.join(" + ")) }
}

fn text_row(ui: &mut egui::Ui, values: &mut HashMap<String, String>, label: &str, key: &str) {
    ui.horizontal(|ui| {
        ui.label(label);
        let mut value = values.get(key).cloned().unwrap_or_else(|| default_value(key));
        if ui.text_edit_singleline(&mut value).changed() { values.insert(key.to_string(), value); }
    });
}

fn combo_row(ui: &mut egui::Ui, values: &mut HashMap<String, String>, label: &str, key: &str, choices: &[&str]) {
    let mut current = values.get(key).cloned().unwrap_or_else(|| default_value(key));
    ui.horizontal(|ui| {
        ui.label(label);
        egui::ComboBox::from_id_source(key).selected_text(&current).show_ui(ui, |ui| {
            for choice in choices { ui.selectable_value(&mut current, (*choice).to_string(), *choice); }
        });
    });
    values.insert(key.to_string(), current);
}

fn slider_row(ui: &mut egui::Ui, values: &mut HashMap<String, String>, label: &str, key: &str, min: f64, max: f64) {
    let mut current = values.get(key).cloned().unwrap_or_else(|| default_value(key)).parse::<f64>().unwrap_or(min);
    if ui.add(egui::Slider::new(&mut current, min..=max).text(label)).changed() { values.insert(key.to_string(), trim_float(current)); }
}

fn clamp_i(value: &str, default: i32, min: i32, max: i32) -> i32 { value.parse::<i32>().unwrap_or(default).clamp(min, max) }
fn clamp_f(value: &str, default: f64, min: f64, max: f64) -> f64 { value.parse::<f64>().unwrap_or(default).clamp(min, max) }
...
fn ensure_dir(path: &Path) -> Result<(), String> { fs::create_dir_all(path).map_err(to_string) }
fn ensure_parent(path: &Path) -> Result<(), String> { path.parent().map(ensure_dir).unwrap_or(Ok(())) }
fn run_quiet(program: &str, args: &[&str]) { let _ = Command::new(program).args(args).stdout(Stdio::null()).stderr(Stdio::null()).spawn(); }
fn to_string<E: std::fmt::Display>(err: E) -> String { err.to_string() }
